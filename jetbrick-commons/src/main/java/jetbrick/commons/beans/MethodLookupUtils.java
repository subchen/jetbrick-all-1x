...
public class MethodLookupUtils {
    ...
    public static MethodDescriptor lookupBestMethodDescriptor(List<MethodDescriptor> methods, String methodName, Class<?>... parameterTypes) {
        MethodDescriptor best = null;
        for (MethodDescriptor method : methods) {
            if (!method.getName().equals(methodName)) continue;

            Class<?>[] types = method.getRawParameterTypes();
            if (isParameterTypesCampatible(types, parameterTypes, method.isVarArgs(), false)) {
                // 可能有多个方法与实际参数类型兼容。采用就近兼容原则。
                if (best == null) {
                    best = method;
                    ...
                    // no change
                } else if (isParameterTypesCampatible(best.getRawParameterTypes(), types, best.isVarArgs(), method.isVarArgs())) {
                    best = method;
                }
            }
        }
    ...
    public static Constructor<?> lookupConstructor(Class<?> clazz, Class<?>... parameterTypes) {
        try {
            ...
            return lookupBestConstructor(Arrays.asList(clazz.getConstructors()), parameterTypes);
        }
    }

    public static Constructor<?> lookupBestConstructor(List<Constructor<?>> constructors, Class<?>... parameterTypes) {
    ...
    public static ConstructorDescriptor lookupBestConstructorDescriptor(List<ConstructorDescriptor> ctors, Class<?>... parameterTypes) {
        ConstructorDescriptor best = null;
        for (ConstructorDescriptor descriptor : ctors) {
            Class<?>[] types = descriptor.getRawParameterTypes();
            if (isParameterTypesCampatible(types, parameterTypes, descriptor.isVarArgs(), false)) {
                // 可能有多个方法与实际参数类型兼容。采用就近兼容原则。
                if (best == null) {
                    best = descriptor;
                } else if (best.isVarArgs() && (!descriptor.isVarArgs())) {
                    best = descriptor; // 不可变参数的函数优先
                } else if ((!best.isVarArgs()) && descriptor.isVarArgs()) {
                    // no change
                } else if (isParameterTypesCampatible(best.getRawParameterTypes(), types, best.isVarArgs(), descriptor.isVarArgs())) {
                    best = descriptor;
                }
            }
        }
